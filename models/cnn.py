...
# INCREASED BATCH SIZE FOR GPU ACCELERATION
...
# 4. Train the Model
print("\nStarting model training on MNIST using GPU...")
model.train()
...
print("\nEvaluating Clean vs FGSM vs PGD attacks on the test set (Accelerated)...")
for images, labels in test_loader:
    ...
    # Save the first sample from the first batch
...
print(f"Accuracy under FGSM attack: {acc_fgsm:.2f}% (Drop: {acc_clean - acc_fgsm:.2f}%)")
print(f"Accuracy under PGD attack:  {acc_pgd:.2f}% (Drop: {acc_clean - acc_pgd:.2f}%)")
print("="*30)
...
plt.tight_layout()
plt.savefig('fgsm_vs_pgd_comparison.png')
print("\nComparative visual saved successfully as 'fgsm_vs_pgd_comparison.png'!")